...
Changes:
    020716  Bugfix BBCODE=>ANSI, bbcode_to_html, ansi_to_html,
            html_to_bbcode, html_to_ansi
    021111  Comment fixes, more case insensitive matching,
            convert deprecated HTML styles (<b>, <i>), more
            space-tolerant matching, img-tags in html_to_bbcode
            works now, some error-preventing stripping and downcasing.

== Examples
=== Converting a string to ANSI and XHTML
    require 'BBCode'
	...
	para=0
	    ...
	    if line.strip.empty?
		  ...
		  "<span>"
	       ...
	       m=$1
	       if $1=~/^(ftp|http|https|mailto|gopher):\/\//
		  m.gsub!('"','&quot;')
		  m.gsub!('>','&gt;')
		  m.gsub!('<','&lt;')
		  "<a href=\"#{m}\">#{m}</a>"
