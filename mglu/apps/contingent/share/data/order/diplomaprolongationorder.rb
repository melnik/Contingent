...
class DiplomaProlongationOrder < StudentsOrder
	TYPE = 'О переносе срока защиты дипломного проекта'
	...
	def try_to_activate
		...
		if DetailsForDiplomaProlongationOrder.has_order?
			fixed_attrs = DetailsForDiplomaProlongationOrder.fix_order(self)
		...
		each_student :all, %w( student_id paragraph_id attributes ) do |student_id, paragraph_id, attributes|
			fixed_attrs = DetailsForDiplomaProlongationOrder.fix_student(self, student_id, paragraph_id, attributes)
